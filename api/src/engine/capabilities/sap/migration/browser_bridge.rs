...
use crate::engine::capabilities::sap::state::ExecuteLoopTurnResult;
...
pub fn resolve_browser_bridge_dir_with_override(explicit: &str) -> String {
    ...
    if !normalized.is_empty() {
        ...
        if let Ok(cwd) = std::env::current_dir() {
            ...
            for dir in cwd.ancestors() {
                ...
                if ancestor_joined.exists() {
                    return ancestor_joined.to_string_lossy().into_owned();
                }
            }
        }
        ...
        if let Some(parent) = candidate.parent() {
            let sibling_bridge = parent.join("bridge");
            if sibling_bridge.exists() {
                return sibling_bridge.to_string_lossy().into_owned();
            }
        }
    }
...
pub fn probe_session(cfg: &mut BrowserTurnConfig) -> Result<crate::engine::capabilities::sap::state::BrowserProbeResult> {
    let mutex = client();
    let mut client = mutex.lock().map_err(|_| anyhow!("Browser bridge mutex poisoned"))?;
